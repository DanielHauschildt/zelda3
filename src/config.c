...
#define M(n)                                        \
  {                                                 \
    #n, kKeys_##n, kKeys_##n##_Last - kKeys_##n + 1 \
  }
#define S(n)         \
  {                  \
    #n, kKeys_##n, 1 \
...
bool ParseBool(const char *value, bool *result)
{
  ...
  case '0':
    ...
    return false;
  case 'f':
    if (StringEqualsNoCase(value, "alse"))
      break;
    return false;
  case 'n':
    if (StringEqualsNoCase(value, "o"))
      break;
    return false;
  case 'o':
    rv = (*value | 32) == 'n';
    if (StringEqualsNoCase(value, rv ? "n" : "ff"))
      break;
    return false;
  case '1':
    rv = true;
  ...
  case 'y':
    rv = true;
    if (StringEqualsNoCase(value, "es"))
      break;
    return false;
  case 't':
    rv = true;
    if (StringEqualsNoCase(value, "rue"))
      break;
    return false;
  default:
    return false;
...
static bool HandleIniConfig(int section, const char *key, char *value)
{
  ...
  else if (section == 1)
  {
    ...
    {
      g_config.output_method = StringEqualsNoCase(value, "SDL-Software") ? kOutputMethod_SDLSoftware : StringEqualsNoCase(value, "OpenGL")  ? kOutputMethod_OpenGL
    ...
    else if (StringEqualsNoCase(key, "ExtendedAspectRatio"))
    {
      ...
      while ((s = NextDelim(&value, ',')) != NULL)
      {
        ...
        { /* Added by daniel.hauschildt */
          ...
          height = atoi(token);
          g_config.extended_aspect_ratio = (h * width / height - 256) / 2;
